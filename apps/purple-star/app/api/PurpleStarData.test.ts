...
describe("PurpleStarData (統合テスト)", () => {
  ...
  const buildBoard = async (cfg: BoardConfig) => {
    ...
    return {
      personalInfo,
      data,
      obj: data.getObject() as Record<string, any>
    };
  };
  ...
  it.each(boardCases)(
    ...
    async ({ config, expected }) => {
      ...
      // 旧暦・数え年
      expect(personalInfo.chineseDate.year).toBe(
        expected.chineseDate.year
      );
      expect(personalInfo.chineseDate.month).toBe(
        expected.chineseDate.month
      );
      expect(personalInfo.chineseDate.day).toBe(
        expected.chineseDate.day
      );
      ...
      expect(main.majorStars.map((s: any) => s.name)).toEqual(
        expected.mainPalace.majorStars
      );
      ...
      for (const { palace, majorStars } of expected.palaceStars) {
        expect(
          palaceByName(obj, palace).majorStars.map((s: any) => s.name)
        ).toEqual(majorStars);
      }
      ...
      if (expected.decadeOrder) {
        for (const { palace, startingAge } of expected.decadeOrder) {
          expect(palaceByName(obj, palace).startingAge).toBe(
            startingAge
          );
        }
      }
      ...
      expect(startingAges).toEqual(
        [...startingAges].sort((a, b) => a - b)
      );
    }
  );
