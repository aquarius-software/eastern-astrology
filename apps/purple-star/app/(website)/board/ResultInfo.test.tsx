...
const renderWith = (overrides: Partial<typeof baseResult> = {}) =>
  render(
    <ResultInfo result={{ ...baseResult, ...overrides } as never} />
  );
...
describe("ResultInfo (purple-star)", () => {
  ...
  it("旧暦が閏月のときは「（閏月）」を付ける", () => {
    renderWith({
      chineseDate: { ...baseResult.chineseDate, isLeapMonth: true }
    });
    expect(valueOf("入力日時（旧暦）")).toBe(
      "己卯年 11月25日 午時（閏月）"
    );
  });
  ...
  it("数え年は生年月日が未来なら非表示、子年斗君は範囲外indexなら空", () => {
    ...
    renderWith({
      birthDateTime: "2030-01-01T03:00:00.000Z",
      januaryBranchIndex: 13
    });
