...
export default function Contact({ settings, ip }) {
  ...
  const { submit: onSubmit } = useWeb3Forms({
    ...
    settings: {
      from_name: "紫微斗数ネクスト",
      subject: `紫微斗数ネクストからお問い合わせが1件あります。${ip}`
    },
    onSuccess: (msg, data) => {
      setIsSuccess(true);
      setMessage(
      ...
      <Container>
        ...
        <div className="grid gap-x-16 md:grid-cols-2">
          <div className="mt-8 mb-2">
            <p className="mt-5">
              機能追加のご要望等、紫微斗数ネクストへのお問い合わせはこちらのフォームからお寄せください。
            </p>
            <p className="mt-5">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="my-10">
              ...
              <div className="mb-3">
                <textarea
                  ...
                  className={`h-40 w-full rounded-md border-2 bg-white px-4 py-3 outline-hidden placeholder:text-gray-800 focus:ring-4 dark:bg-gray-900 dark:text-white dark:placeholder:text-gray-200 ${
                    errors.message
                      ? "border-red-600 ring-red-100 focus:border-red-600 dark:ring-0"
                      : "border-gray-300 ring-gray-100 focus:border-gray-600 dark:border-gray-600 dark:ring-0 dark:focus:border-white"
                  }`}
