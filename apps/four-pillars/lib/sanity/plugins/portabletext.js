...
const getComponents = (headings, subHeadings) => {
  const components = {
    block: {
      h2: ({ value, children }) => {
        if (headings) {
          const heading = headings.find(h => h._key === value._key);
      ...
      h3: ({ value, children }) => {
        if (subHeadings) {
          const heading = subHeadings.find(
            h => h._key === value._key
          );
    ...
    marks: {
      ...
      internalLink: ({ children, value }) => {
        return (
          <Link href={`/post/${value.slug.current}`} prefetch={false}>
            {children}
          </Link>
        );
      }
...
export const PortableText = props => (
  <PortableTextComponent
    components={getComponents(props.headings, props.subHeadings)}
    {...props}
  />
);
