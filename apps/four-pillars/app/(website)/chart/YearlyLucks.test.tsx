...
describe("YearlyLucks", () => {
  ...
  it("取得後、各歳運の干支・大運・西暦年・満年齢を整形表示する", async () => {
    ...
    expect(
      within(row("甲辰")).getByText("2024〜25")
    ).toBeInTheDocument(); // 西暦年
    expect(
      within(row("甲辰")).getByText("24〜25歳")
    ).toBeInTheDocument(); // 満年齢

    expect(
      within(row("乙巳")).getByText("甲戌-乙亥")
    ).toBeInTheDocument(); // 大運（複数を - 連結）
    expect(
      within(row("乙巳")).getByText("2025〜26")
    ).toBeInTheDocument();
  });
  ...
  it("「通変星」をオンにすると通変星の列が表示される", async () => {
    ...
    expect(
      screen.getByRole("columnheader", { name: "通変星" })
    ).toBeInTheDocument();
  ...
  it("「命式との関係」をオンにすると関係列が表示され、空亡が示される", async () => {
    ...
    fireEvent.click(
      screen.getByRole("checkbox", { name: "命式との関係" })
    );
    expect(
      screen.getByRole("columnheader", { name: "命式との関係" })
    ).toBeInTheDocument();
