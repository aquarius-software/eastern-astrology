  ...
  return (
    <div className="section-container">
      ...
      <table className="luck-table border-collapse shadow">
        <thead className="font-medium text-gray-600 dark:divide-gray-600 dark:text-white">
          <tr className="dark:text-gray-300">
            <th className="table-top-header">干支</th>
            ...
            {showTwelveLucks && (
              <th className="table-top-header">十二運</th>
            )}
          </tr>
        </thead>
        <tbody className="text-gray-600 dark:divide-gray-600 dark:text-white">
          {decadeLucks.map((item: DecadeLuck, i: number) => (
            <React.Fragment key={i}>
              <tr
                data-tooltip-id={
                  item.inCurrentPeriod
                    ? "showCurrentDecadeLuckTooltip"
                    : undefined
                }
                ...
                <td className="table-cell-content border border-slate-300 font-bold dark:bg-gray-600">
                  <button
                    className={`mr-1 h-7 w-7 cursor-default rounded-full font-bold text-black ${
                ...
                {showRelationships && (
                  <td className="table-cell-content">
                    <span className="block">
                      {stemCombinationStr(item)}
                    </span>
                    <span className="block">
                      {branchCombinationStr(item)}
                    </span>
                    <span className="block">
                      {branchClashesStr(item)}
                    </span>
                    <span className="block">
                      {threeHarmonyCombinationsStr(item)}
                    </span>
      ...
      <Tooltip
        ...
        id="showCurrentDecadeLuckTooltip"
        place="top"
      />
      <div className="mb-4">
