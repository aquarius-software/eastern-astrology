...
describe("DecadeLucks", () => {
  it("既定では干支・開始年月・満年齢の3列を表示する", () => {
    ...
    // 切り替え列は既定で非表示
    expect(
      screen.queryByRole("columnheader", { name: "通変星" })
    ).toBeNull();
    expect(
      screen.queryByRole("columnheader", { name: "十二運" })
    ).toBeNull();
    expect(
      screen.queryByRole("columnheader", { name: "命式との関係" })
    ).toBeNull();
  });
  ...
  it("各大運の干支・開始年月・満年齢を整形表示する", () => {
    render(<DecadeLucks result={result} />);
    expect(
      within(row("甲戌")).getByText("2000年1月")
    ).toBeInTheDocument();
    expect(within(row("甲戌")).getByText("0歳")).toBeInTheDocument();
    expect(
      within(row("乙亥")).getByText("2005年3月")
    ).toBeInTheDocument();
  ...
  it("「十二運」をオンにすると十二運の列が表示される", () => {
    ...
    expect(
      screen.getByRole("columnheader", { name: "十二運" })
    ).toBeInTheDocument();
