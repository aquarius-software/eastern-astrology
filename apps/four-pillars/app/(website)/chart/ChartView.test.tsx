...
describe("ChartView", () => {
  ...
  it("四柱を正しい干支のアイコン(aria-label)で表示する", () => {
    ...
    expect(
      screen.getByRole("img", { name: "己" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("img", { name: "卯" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("img", { name: "丙" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("img", { name: "子" })
    ).toBeInTheDocument();
    // 日柱・時柱はどちらも戊午 → 戊・午は2つずつ出る
    expect(screen.getAllByRole("img", { name: "戊" })).toHaveLength(
      2
    );
    expect(screen.getAllByRole("img", { name: "午" })).toHaveLength(
      2
    );
  });
  ...
  it("ニックネームの有無でヘッダー表記が変わる", () => {
    const { unmount } = render(
      <ChartView result={buildResult(false, "太郎")} />
    );
  ...
  it("生時不明の場合は時柱を描画しない", () => {
    const { container } = render(
      <ChartView result={buildResult(true, "")} />
    );
    // 「生時不明」のツールチップ付きセルになる
    expect(
      container.querySelector('[data-tooltip-content="生時不明"]')
    ).not.toBeNull();
