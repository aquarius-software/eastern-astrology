...
export default function ResultTime({
  ...
  const localAdjustedDateTime = adjustedDateTime.toUTC(
  ...
  // 節入り日時
  const currentTermStart = DateTime.fromISO(
    currentTerm.startTime.toString()
  );
  const localCurrentTermStart = currentTermStart.toUTC(
    Number(-timezoneOffset)
  );
  const localCurrentTermStartStr =
    localCurrentTermStart.toFormat("y年M月d日 H:mm");

  // 翌月節入り日時
  const currentTermEnd = DateTime.fromISO(
    currentTerm.endTime.toString()
  );
  const localCurrentTermEnd = currentTermEnd.toUTC(
    Number(-timezoneOffset)
  );
  const localCurrentTermEndStr =
    localCurrentTermEnd.toFormat("y年M月d日 H:mm");
    ...
    <div className="section-container">
      <table className="section-table">
        <tbody className="section-table-body">
          ...
          <tr className="table-row">
            <td className="table-left-header">均時差</td>
            <td className="table-cell-content-without-border">
              {/* {equationOfTime.neg ? "-" : "+"}
              {equationOfTime.m}分
          ...
          <tr className="table-row">
            <td className="table-left-header">調整後日時</td>
            <td className="table-cell-content-without-border">
              {`${localAdjustedDateTimeStr}`}
            </td>
          </tr>
          <tr className="table-row">
            <td className="table-left-header">節入り日時</td>
            <td className="table-cell-content-without-border">
              {localCurrentTermStartStr}
            </td>
          </tr>
          <tr className="table-row">
            <td className="table-left-header">翌月節入り日時</td>
            <td className="table-cell-content-without-border">
              {localCurrentTermEndStr}
            </td>
          </tr>
          <tr className="table-row">
            <td className="table-left-header">節入り後日数</td>
            <td className="table-cell-content-without-border">
              {elapsedDaysNum.toFixed(1)}日
            </td>
          </tr>
          <tr className="table-row">
            <td className="table-left-header">太陽黄経</td>
            <td className="table-cell-content-without-border">
              {eclipticLongitude.toFixed(2)}度
            </td>
          </tr>
          <tr className="table-row">
            <td className="table-left-header">二十四節気</td>
            <td className="table-cell-content-without-border">
              {`${solarTerm}${earthPeriodStr}`}
            </td>
          </tr>
          <tr className="table-row">
            <td className="table-left-header">二十四節気分割法</td>
            <td className="table-cell-content-without-border">
              {useSpaceMethod ? "定気法" : "平気法（恒気法）"}
            </td>
          </tr>
          <tr className="table-row">
            <td className="table-left-header">日柱切り替え時刻</td>
            <td className="table-cell-content-without-border">
