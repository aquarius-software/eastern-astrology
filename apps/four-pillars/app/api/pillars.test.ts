...
const b = (value: string) =>
  EARTHLY_BRANCHES.find(x => x.value === value)!;
/** 天干の value から HEAVENLY_STEMS のオブジェクトを引くヘルパー */
const s = (value: string) =>
  HEAVENLY_STEMS.find(x => x.value === value)!;
...
describe("getYearPillar", () => {
  ...
  it("2024年は甲辰(index=41)、2023年は癸卯(index=40)", () => {
    expect(getYearPillar(2024)).toMatchObject({
      index: 41,
      stem: "甲",
      branch: "辰"
    });
    expect(getYearPillar(2023)).toMatchObject({
      index: 40,
      stem: "癸",
      branch: "卯"
    });
  ...
  it("(year-3)が60で割り切れる場合は index=60(癸亥)に補正される", () => {
    ...
    expect(getYearPillar(3)).toMatchObject({
      index: 60,
      stem: "癸",
      branch: "亥"
    });
  ...
  it("(year-3)が負になる場合も正しく循環する", () => {
    ...
    expect(getYearPillar(2)).toMatchObject({
      index: 59,
      stem: "壬",
      branch: "戌"
    });
...
describe("getMonthPillar", () => {
  it("2024年2月(立春後)は丙寅(index=3)", () => {
    expect(getMonthPillar(2024, 2)).toMatchObject({
      index: 3,
      stem: "丙",
      branch: "寅"
    });
...
describe("getDayPillar", () => {
  ...
  it("23時かつ changeDayStem=true なら翌日の干支(己未, index=56)になる", () => {
    expect(getDayPillar(23, JD_2000_01_01, true)).toMatchObject({
      index: 56
    });
  });

  it("23時でも changeDayStem=false なら当日の干支のまま(戊午)", () => {
    expect(getDayPillar(23, JD_2000_01_01, false)).toMatchObject({
      index: 55
    });
  });
});
...
describe("getHourPillar", () => {
  it("甲日の0時(子刻)は甲子(index=1)", () => {
    expect(getHourPillar(0, "甲", false)).toMatchObject({
      index: 1,
      stem: "甲",
      branch: "子"
    });
...
describe("getThreeHarmonyBranches", () => {
  it("申・子・辰は三合水局", () => {
    expect(
      getThreeHarmonyBranches([b("申"), b("子"), b("辰")])?.name
    ).toBe("三合水局");
  });

  it("三合が成立しない組み合わせは undefined", () => {
    expect(
      getThreeHarmonyBranches([b("子"), b("丑"), b("寅")])
    ).toBeUndefined();
  });
});
...
describe("getThreeSeasonalBranches", () => {
  it("寅・卯・辰は東方合", () => {
    expect(
      getThreeSeasonalBranches([b("寅"), b("卯"), b("辰")])?.name
    ).toBe("東方合");
  });
});
...
describe("getTwoHarmonyBranches", () => {
  it("子・辰は三合水局半会", () => {
    expect(
      getTwoHarmonyBranches([b("子"), b("辰")]).map(c => c.name)
    ).toEqual(["三合水局半会"]);
  });
...
describe("getTwoSeasonalBranches", () => {
  it("寅・卯は東方合半会", () => {
    expect(
      getTwoSeasonalBranches([b("寅"), b("卯")]).map(c => c.name)
    ).toEqual(["東方合半会"]);
  });
});
...
describe("getTwoHarmonyBranchesWithDecadeLuck", () => {
  it("命式の申と大運の子で三合水局半会", () => {
    expect(
      getTwoHarmonyBranchesWithDecadeLuck([b("申")], b("子")).map(
        c => c.name
      )
    ).toEqual(["三合水局半会"]);
  });
...
describe("getTwoSeasonalBranchesWithDecadeLuck", () => {
  it("命式の寅と大運の卯で東方合半会", () => {
    expect(
      getTwoSeasonalBranchesWithDecadeLuck([b("寅")], b("卯")).map(
        c => c.name
      )
    ).toEqual(["東方合半会"]);
  });
...
describe("getBranchPairs", () => {
  it("子と丑は支合", () => {
    expect(
      getBranchPairs([b("子"), b("丑")]).map(p => p.name)
    ).toEqual(["子丑"]);
  });
...
describe("getBranchBreaks", () => {
  it("子と酉は破", () => {
    expect(
      getBranchBreaks([b("子"), b("酉")]).map(p => p.name)
    ).toEqual(["子酉"]);
  });
});
...
describe("getBranchHarms", () => {
  it("子と未は害", () => {
    expect(
      getBranchHarms([b("子"), b("未")]).map(p => p.name)
    ).toEqual(["子未"]);
  });
});
...
describe("getBranchClashes", () => {
  it("子と午は冲", () => {
    expect(
      getBranchClashes([b("子"), b("午")]).map(c => c.name)
    ).toEqual(["子午"]);
  });
...
describe("getStemPairs", () => {
  it("甲と己は干合", () => {
    expect(getStemPairs([s("甲"), s("己")]).map(p => p.name)).toEqual(
      ["甲己"]
    );
  });
});
...
describe("getBranchPunishments", () => {
  it("寅と巳は恃勢の刑", () => {
    ...
    expect(punishments[0]).toMatchObject({
      name: "寅巳",
      type: "恃勢の刑"
    });
  });
...
describe("getRoots", () => {
  ...
  it("stemRoots が空なら空配列", () => {
    expect(
      getRoots([], undefined, undefined, undefined, undefined)
    ).toEqual([]);
    expect(
      getRoots(undefined, undefined, undefined, undefined, undefined)
    ).toEqual([]);
  });
});
