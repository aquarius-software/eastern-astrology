...
describe("FourPillarsData (統合テスト)", () => {
  ...
  it.each(chartCases)(
    "$name の四柱・節気・行運・五行を算出する",
    ({ config, expected }) => {
      const data = buildChart(config);
      const fp = data.getFourPillars();

      expect(`${fp.year.stem}${fp.year.branch}`).toBe(expected.year);
      expect(`${fp.month.stem}${fp.month.branch}`).toBe(
        expected.month
      );
      expect(`${fp.day.stem}${fp.day.branch}`).toBe(expected.day);
      expect(`${fp.hour!.stem}${fp.hour!.branch}`).toBe(
        expected.hour
      );

      expect(data.personalInfo.solarTerm?.name).toBe(
        expected.solarTerm
      );
      expect(data.luckOrder).toBe(expected.luckOrder);
      // 浮動小数の誤差を避けるため小数2桁に丸めて比較
      expect(
        data.elementComposition.map(n => Number(n.toFixed(2)))
      ).toEqual(expected.elementComposition);
  ...
  it("getObject() は主要な命式情報を含むオブジェクトを返す", () => {
    const obj = buildChart({ ...BASE }).getObject() as Record<
      string,
      unknown
    >;
