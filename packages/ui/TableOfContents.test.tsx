...
describe("TableOfContents", () => {
  it("各見出しを #id へのアンカーリンクとして描画する", () => {
    ...
    expect(screen.getByRole("link", { name: "本論" })).toHaveAttribute(
      "href",
      "#main",
    );
  });
  ...
  it("subheadings を再帰的にネスト描画する", () => {
    ...
    expect(screen.getByRole("link", { name: "詳細1" })).toHaveAttribute(
      "href",
      "#main-1",
    );
    // 「詳細1」は「本論」の <li> 配下の入れ子 <ol> の中にある
    const nested = screen.getByRole("link", { name: "詳細1" }).closest("ol");
    expect(nested?.closest("li")).toContainElement(
      screen.getByRole("link", { name: "本論" }),
    );
  });
  ...
  it("subheadings が空の見出しは入れ子の目次を持たない", () => {
    render(
      <TableOfContents
        outline={[{ id: "only", text: "単独", subheadings: [] }]}
      />,
    );
